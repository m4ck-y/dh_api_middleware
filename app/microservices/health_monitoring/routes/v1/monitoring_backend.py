...
async def backend_health():
    status, data = await request(HEALTH_MONITORING_URL, "GET", "health")
    if status >= 400:
...
async def backend_health_detailed():
    status, data = await request(HEALTH_MONITORING_URL, "GET", "health/detailed")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    return data
...
async def backend_version():
    status, data = await request(HEALTH_MONITORING_URL, "GET", "version")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
...
async def backend_migrate():
    status, data = await request(HEALTH_MONITORING_URL, "POST", "admin/migrate")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    if isinstance(data, dict):
...
async def backend_clear_cache():
    status, data = await request(HEALTH_MONITORING_URL, "POST", "admin/clear-cache")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    if isinstance(data, dict):
        return JsonObjectResponse.model_validate(data)
