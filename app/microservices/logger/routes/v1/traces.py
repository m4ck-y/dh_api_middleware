...
from app.microservices.logger.domain.models import TraceEntry
...
async def get_traces(
    ...
    status_code, data = await request(
        LOGGER_URL, "GET", "traces", params=params
    )
    if status_code >= 400:
...
async def create_trace(payload: TraceEntry):
    ...
    status_code, data = await request(
        LOGGER_URL, "POST", "traces", json=payload.model_dump(mode="json")
    )
    if status_code >= 400:
        raise HTTPException(status_code=status_code, detail=data)
    return data
...
async def create_traces_batch(payload: List[TraceEntry]):
    ...
    status_code, data = await request(
        LOGGER_URL, "POST", "traces/batch", json=[item.model_dump(mode="json") for item in payload]
    )
    if status_code >= 400:
        raise HTTPException(status_code=status_code, detail=data)
    return data
