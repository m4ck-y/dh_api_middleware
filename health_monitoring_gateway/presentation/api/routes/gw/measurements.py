...
async def list_measurements(
    ...
    status, data = await request(
        "GET", "measurements", params=params if params else None
    )
    if status >= 400:
...
async def get_measurement(measurement_id: int):
    status, data = await request("GET", f"measurements/{measurement_id}")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    return data
...
async def create_measurement(payload: MeasurementCreate):
    status, data = await request("POST", "measurements", json=payload.model_dump())
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    return data
...
async def update_measurement(measurement_id: int, payload: MeasurementCreate):
    status, data = await request(
        "PUT", f"measurements/{measurement_id}", json=payload.model_dump()
    )
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    return data
...
async def delete_measurement(measurement_id: int):
    status, data = await request("DELETE", f"measurements/{measurement_id}")
    if status >= 400:
        raise HTTPException(status_code=status, detail=data)
    return data
